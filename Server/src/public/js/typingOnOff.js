...
$(document).ready(function(){
  socket.on("response-user-is-typing",function(response){
    ...
    let messageTyping=`<div class="bubble you bubble-typing-gif">x
  ...
  socket.on("response-user-is-not-typing",function(response){
    if(response.currentGroupId){
      if(response.currentUserId !== $("#dropdown-navbar-user").data("uid")){
        $(`.chat[data-chat=${response.currentGroupId}]`).find("div.bubble-typing-gif").removeClass();
      ...
      $(`.chat[data-chat=${response.currentUserId}]`).find("div.bubble-typing-gif").removeClass();
        nineScrollRight(response.currentUserId);
    }
  })
})
